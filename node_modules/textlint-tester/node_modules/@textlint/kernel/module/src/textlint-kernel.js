...
export class TextlintKernel {
    ...
    async _parallelProcess({ descriptor, text, options }) {
        ...
        invariant(typeof preProcess === "function" && typeof postProcess === "function", `${plugin.id} processor should implements {preProcess, postProcess}`);
        ...
        if (preProcessResult instanceof Error) {
            return createDummyTextLintResult(`Failed to parse text by plugin: ${plugin.id}
...
`, filePath);
        ...
        if (coreFlags.runningTester) {
            invariant(isTxtAST(ast), `${plugin.id} processor return invalid AST object. Please check out @textlint/ast-tester.
